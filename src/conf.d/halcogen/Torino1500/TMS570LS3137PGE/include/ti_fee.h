...
#define TI_FEE_SW_MAJOR_VERSION 0U
#define TI_FEE_SW_MINOR_VERSION 1U
#define TI_FEE_SW_PATCH_VERSION 11U
...
typedef uint32 TI_Fee_AddressType;                     /* Used for defining variables to indicate number of bytes for address offset */
typedef uint32 TI_Fee_LengthType;                      /* Used for defining variables to indicate number of bytes per read/write/erase */
...
{   
   uint32  FeeIndex;                                 /* Instance ID of this module. Should always be 0 */
                                                     /* Fixed Value: 0 */   
   boolean FeeEnableErrorCorrection;                 /* Enables use of Error correction if supported by underlying device */
                                                     /* Default: STD_OFF */
   uint16  FeeVirtualPageSize;                       /* Defines the virtual page size.  Our implementation is currently a static 8 byte Page Size */
                                                     /* Fixed value: 8 */        
   boolean FeeGetJobResultApi;                        /* Enables Fee_GetJobResult() function */
                                                     /* Default: STD_ON */
   uint16  FeeFrequency;                              /* Operating Frequency in MHz */
                                                     /* Default: 0-not valid forcing user to select value */
                                                     /* Min: 10 (MHz); Max: 500 (MHz) */
} TI_Fee_GeneralConfigType;
...
typedef struct
{
   ...
   uint16  FeeFlashBank;                             /* Flash Bank to use for virtual sector. */
                                                     /* As we do not allow Flash EEPROM Emulation in Bank 0, 0 is not a valid option */
                                                     /* Defaultvalue 1, Minimum 1, Maxiumum 7 */   
   Fapi_FlashSectorType FeeStartSector;              /* Defines the Starting Sector in the Bank for this Virtual Sector */
   Fapi_FlashSectorType FeeEndSector;                /* Defines the Ending Sector in the Bank for this Virtual Sector */  
                                                     /* Start and End sectors can be the same, which indicates only one sector */
                                                     /* is the entire virtual sector. */
                                                     /* Values are based on the FLASH_SECT enum */
                                                     /* Defaultvalue and Min is the same sector defined as the starting sector */
                                                     /* Max values are based on the device definition file being used. */
...
typedef struct
{
   ...
   uint32  FeeNumberOfWriteCycles;                   /* Number of write cycles this block requires */
                                                     /* Default: 0, but this will not be a valid number.  Force customer to select a value */
                                                     /* Min 1, Max (2^32)-1 */
...
typedef struct
{
   ...
   uint8   FeeVirtualSectorOverhead;                 /* Virtual Sector overhead in bytes */
                                                     /* Fixed Value: 16 */
} TI_Fee_PublishedInformationType;
...
typedef struct
{
	...
	TI_Fee_AddressType Fee_oCopyCurrentAddress;        		/* Indicates the Address within the Active VS which will be copied to Copy VS */
	TI_Fee_AddressType Fee_oCopyNextAddress;			 	/* Indicates the Address within the Copy VS to which the data from Active VS will be copied to */
	TI_Fee_AddressType Fee_u32nextwriteaddress;				/* Indicates the next free Address within the curent VS to which the data will be written */	
	...
	TI_Fee_AddressType Fee_oCurrentAddress;			 		/* Indicates the current address of the Virtual Sector Header being written */
	TI_Fee_AddressType Fee_oCurrentStartAddress;		 	/* Start Address of the Previous Block	 */
	TI_Fee_AddressType Fee_oCurrentBlockHeader;		 		/* Start Address of the Block which is being currently written*/
	...
	uint16 Fee_u16CopyBlockSize;					 		/* Indicates the size of current block in bytes which is been copied from Active to Copy VS */
	...
	uint32 Fee_au32VirtualSectorStateValue[TI_FEE_VIRTUAL_SECTOR_OVERHEAD >> 2U];  		/* Array to store the Virtual Sector Header and Information record */		
	uint8 Fee_au8VirtualSectorState[TI_FEE_NUMBER_OF_VIRTUAL_SECTORS]; 					/* Stores the state of each Virtual sector */
	uint32 Fee_au32VirtualSectorEraseCount[TI_FEE_NUMBER_OF_VIRTUAL_SECTORS]; 	 		/* Array to store the erase count of each Virtual Sector */	
	TI_Fee_AddressType Fee_au32BlockOffset[TI_FEE_NUMBER_OF_BLOCKS][(1U<< TI_FEE_DATASELECT_BITS)]; /* Array to store the Block Offset within the VS */		
	uint32 Fee_au32BlockHeader[TI_FEE_BLOCK_OVERHEAD >> 2U];								/* Array to store the Block Header value */
	uint8  Fee_au8BlockCopyStatus[TI_FEE_NUMBER_OF_BLOCKS][(1U<<TI_FEE_DATASELECT_BITS)]; 	/* Array to store block copy status */	
	...
	uint16 Fee_u16DataSetBits;						/* Indicates the number of bits in the Block Number used for DataSets */		
	uint16 Fee_u16InternalEraseQueue;				/* Indicates which VS can be erased when the FEE is in BusyInternal State*/			
	...
	uint8 Fee_u8WriteCopyVSHeader;					/* Indicates the number of bytes of the Copy VS Header being written */
	uint8 Fee_u8WriteCount;							/* Indicates the number of bytes of the Block Header being written */
	...
	boolean Fee_bInvalidWriteBit;					/* Indicates whether the block is written/invalidated/erased for the first time */
	boolean Fee_bWriteData;							/* Indicates that there is data which is pending to be written to the Block */	
	...
	TI_Fee_AddressType Fee_au32UnConfiguredBlockAddress[TI_FEE_NUMBER_OF_UNCONFIGUREDBLOCKSTOCOPY];		/* Indicates number of unconfigured blocks to copy */
	uint8  Fee_au8UnConfiguredBlockCopyStatus[TI_FEE_NUMBER_OF_UNCONFIGUREDBLOCKSTOCOPY]; 	/* Array to store block copy status */
	#endif
...
extern const TI_Fee_GeneralConfigType TI_Fee_General;
extern const TI_Fee_PublishedInformationType TI_Fee_PublishedInformation;
...
/*SAFETYMCUSW 61 X MR:1.4,5.1 <INSPECTED> "Reason -  TI_FeeInternal_GetVirtualSectorParameter name is required here."*/
...
/*SAFETYMCUSW 61 X MR:1.4,5.1 <INSPECTED> "Reason -  TI_FeeInternal_GetVirtualSectorIndex name is required here."*/
