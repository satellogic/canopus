...
#include <setjmp.h> // XXX optimize embedded?
...
#ifdef _CANOPUS /* FIXME I'd rather have this by default (phil) */
# include <canopus/cpu.h>
# define exit(errcode) cpu_reset2("CPU HALT", errcode) /* undefined reference to `_fini' */
#endif
...
static int memory_not_equal_display_error(const char* a, const char* b,
	...
	for (i = 0; i < size; i++) {
		...
		const char r = b[i];
		if (l == r) {
			/* Avoid printing errors when no error was really found */
...
void vprint_message(const char* const format, va_list args) {
	...
	vsnprintf(buffer, sizeof(buffer), format, args);
        log_report_fmt(LOG_GLOBAL, "%s MSG -- %s"CARRIAGE, CMOCKERY_ID_STRING, buffer);
#ifdef _WIN32
...
void vprint_error(const char* const format, va_list args) {
	char buffer[1024];
	vsnprintf(buffer, sizeof(buffer), format, args);
        log_report_fmt(LOG_GLOBAL, "%s ERR -- %s"CARRIAGE, CMOCKERY_ID_STRING, buffer);
#ifdef _WIN32
	OutputDebugString(buffer);
#endif // _WIN32
}
